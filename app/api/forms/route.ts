...
export async function POST(req: Request) {
  ...
  try {
    await resend().emails.send({ from: FROM, to: INBOX, replyTo: /@/.test(email) ? email : undefined, subject: `[AFIA site] ${form}`, html: `<h2 style="font-family:sans-serif">${esc(form)}</h2><table style="font-family:sans-serif;font-size:14px;border-collapse:collapse">${rows}</table>` });
